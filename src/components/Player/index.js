...
const Image = ({ src }) => {
  ...
  return (
    <div>
      <img
        onMouseEnter={() => {console.log('entering'); setHovering(true)}}
        onMouseLeave={() => {console.log('exit'); setHovering(false); }}
...
const Description = ({ title, subTitle }) => {
  return (
    <div>
      <h3 className='text-2xl text-grey-darkest font-medium'>{title}</h3>
...
const Overlay = () => {
  return <div className='w-full rounded hidden md:block'>Overlay</div>;
};
